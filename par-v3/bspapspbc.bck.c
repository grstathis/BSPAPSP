...
int s,t,i,j,k,l,li,lsize,tsize0, tsize1,tempp,tempoff,rpos,cpos, 
...
lsize = nloc(bsp_nprocs(),bsp_pid(),N*N); //Get the number of elements of processor s
...
if ( pn  != pm ){
  ...
  if( bsp_pid()==0){
	...
	printf("I am proc 2 and i should have s = %d and t = %d",s,t);
  }
}else{
	s= bsp_pid()%pm;  /* 0 <= s < pm */
	t= bsp_pid()/pn;  /* 0 <= t < pn */
...
//printf("I am proc %d and i have s = %d and t = %d", bsp_pid(),s,t);
...
for ( i= 0; i < N; i++){
	for ( j= 0; j < N; j++){
		if (( (i % pm) == s) && ((j % pn) == t)){
			//printf("I am proc %d and i have s = %d and t = %d with i %d and j %d \n", bsp_pid(),s,t,i,j);
			lpart[li] = init[N*i+j];
			lrow[li] = i;
			lcol[li] = j;
			gindx[li] = N*i+j;
